 ...
 *   const keywords = generateSearchKeywords('2 Bedroom Apartment in Kampala');
 */

const ABBREVIATIONS = {
  bedroom: ['br', 'bdrm', 'bed'],
  bedrooms: ['br', 'bdrm', 'beds'],
  bathroom: ['bath', 'bth'],
  bathrooms: ['bath', 'bth', 'baths'],
  apartment: ['apt', 'apartment', 'flat'],
  'for rent': ['rent', '4rent', 'to-let', 'to let'],
...
export function generateSearchKeywords(title, opts = {}) {
  ...
  // Common marketplace patterns: numeric-bedroom shorthand (e.g., '2br', '2-bedroom')
  for (let i = 0; i < tlen; i++) {
    const w = tokens[i];
    const next = tokens[i + 1];
    if (/^\d+$/.test(w) && next) {
      // e.g., '2 bedroom' -> '2br', '2-bedroom'
      if (/bed(room)?s?/.test(next)) {
        out.push(`${w}br`);
        out.push(`${w}-br`);
        out.push(`${w} bedroom`);
        out.push(`${w}-bedroom`);
      }
      if (/bath(room)?s?/.test(next)) {
        out.push(`${w}bath`);
        out.push(`${w}-bath`);
      }
    }
  ...
  // Per-token combos with abbreviations (e.g., '2br apartment' -> '2br apt')
  // If title contains a number + bedroom and apartment/flat tokens, craft combos
  const hasNumber = tokens.find((t) => /^\d+$/.test(t));
  const hasBedroom = tokens.find((t) => /bed(room)?s?/.test(t));
  const hasApartment = tokens.find((t) => /(apartment|flat|apt)/.test(t));
  if (hasNumber && hasBedroom) {
    const num = hasNumber.match(/^\d+$/)[0];
    if (hasApartment) {
      out.push(`${num}br apt`);
      out.push(`${num}br flat`);
    }
  }
